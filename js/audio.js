...
var notes = [443.55, 469.92, 352.04]; //D, D#, G
//LOOP FUNCTION
function loopFunction(next) {
	if (loopIsPlaying) {
		if (isFirstPattern) {
			  //kick(next + (oneBar / 2), 55.44, 0.005, 10, 0.01);
			 // kick(next + (oneBar / 2), 50, 0.005, 0.5, 0.01);
			 // kick(next + (oneBar / 1.5), 50, 0.005, 0.5, 0.01);
			

			 //lead(next + (oneBar / 6), notes[0], 0.01, (Math.random() * 0.3) + 0.15);
			 // lead(next + (oneBar / 1.3), notes[2] * 2, 0.01, (Math.random() * 0.6) + 0.3);
			 // lead(next + (oneBar / 3), notes[1], 0.01, (Math.random() * 0.3) + 0.15);
